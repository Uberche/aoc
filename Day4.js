// let input1 = `sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp
...
// console.log(aInput.map((a) => a.split(' ')).filter((b) => !b.some((x,i) => b.find((y,j) => (i  != j) && (x == j) ))).length);
...
let sen = "iiii oiii ooii oooi oooo";
// oiii ioii iioi iiio`;

let letters;
sen = sen.split(' ');
sen = sen.map((word,ind) => wordToInd(word));

for (let first = 0; first < sen.length-1; first++) {
  let firstSet = Object.keys(sen[first]);
  for (let second = first +1;  second<sen.length; second++){
    let secondSet = Object.keys(sen[second]);
    if (firstSet.length < 2) {
      let ind = secondSet.indexOf(firstSet[0])
      let val = secondSet[ind];
      console.log(secondSet[i]);
      // if (ind != -1 && secondSet[ind] === ) {
        
      // }
    }
    // if (Object.keys(sen[second]).includes(Object.keys(sen[second])) {
    //   console.log("ok...");
    // }
  }
}
...
function wordToInd(word) {
  let count = 1;
  let letters = {};
  word = word.split('');
  word = word.forEach((y) => {
    if (!Object.keys(letters).includes(y)) {
      letters[y] = count;
    } else {
